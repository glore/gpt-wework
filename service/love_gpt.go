...
func LoveGPTChat(c *gin.Context) {
	...
	gptType := c.PostForm("gptType") //love小恋 cherub小天使
	if len(question) == 0 {
	...
	//	return
	//}
	ret, err := LoveGPTService(question, sessionId, model, contextMsg, gptType)
	if err != nil {
...
func LoveGPTService(question, sessionId string, model string, contextMsg string, gptType string) (string, error) {
	...
	defer chat.ctx.Done()
	answer, err := chat.Request(messages,gptType)
	if err != nil {
		fmt.Print(err.Error())
	}
	return answer, err
...
func (c *LoveGPT) Request(messages []gogpt.ChatCompletionMessage, gptType string ) (answer string, err error) {
